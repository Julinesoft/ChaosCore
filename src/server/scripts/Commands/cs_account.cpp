...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "expansion",      SEC_ADMINISTRATOR,  true,  &HandleAccountSetAddonCommand,     "", NULL },
        ...
        static ChatCommand accountCommandTable[] =
        {
            ...
            { "votepoints",     SEC_MODERATOR,      false, &HandleAccountGetVotepointsCommand,"", NULL },
    ...
    static bool HandleAccountGetVotepointsCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;

        char *accNameStr=strtok((char*)args," ");
        if (!accNameStr)
            return false;

        std::string accName = accNameStr;
        if (!AccountMgr::normalizeString(accName))
        {
            handler->PSendSysMessage(LANG_ACCOUNT_NOT_EXIST,accName.c_str());
            return false;
        }

        uint32 accId = sAccountMgr->GetId(accName);
        if (!accId)
        {
            handler->PSendSysMessage(LANG_ACCOUNT_NOT_EXIST,accName.c_str());
            return false;
        ...
        //                                                0
        QueryResult result = LoginDatabase.PQuery("select web_points from mw_account_extend where account_id=%u", accId);
        Field *fields = result->Fetch();
        vp = fields[0].GetUInt32();

        handler->PSendSysMessage("Account: '%s' has %u VotePoints.", accName.c_str(), vp);
        return true;
    }
