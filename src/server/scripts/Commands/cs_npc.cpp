...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        static ChatCommand npcCommandTable[] =
        {
            ...
            { "spawn",          SEC_GAMEMASTER,     false, &HandleNpcAddTempSpawnCommand,      "", NULL },
            { "permspawn",      SEC_GAMEMASTER,     false, &HandleNpcAddCommand,               "", NULL },
    ...
    #define TIME_BETWEEN_PERM_SPAWNS_MILLIS 5000
    static bool HandleNpcAddCommand(ChatHandler* handler, const char* args)
    {
	    ...
	    {
            if (!handler->GetSession()->m_lastSpawnTime)
                handler->GetSession()->m_lastSpawnTime = 0;
		    int timeSinceLastSpawn = handler->GetSession()->m_lastSpawnTime - currMsTime;
		    if (timeSinceLastSpawn > -TIME_BETWEEN_PERM_SPAWNS_MILLIS && timeSinceLastSpawn < TIME_BETWEEN_PERM_SPAWNS_MILLIS)
		    {
			    handler->PSendSysMessage("You may only spawn NPCs once every 5 seconds.");
			    return true;
		    }
		    else
		    {
			    handler->GetSession()->m_lastSpawnTime = currMsTime;
		    }
	    }
        ...
        if (!handler->GetSession()->GetPlayer()->CanSpawnNPC(id) && !handler->GetSession()->GetPlayer()->IsAdmin())
        {
            handler->PSendSysMessage("You cannot spawn this NPC.");
            return true;
        }
        ...
        if (chr->GetZoneId() == 876 && !handler->GetSession()->GetPlayer()->IsAdmin())
        {
            handler->PSendSysMessage("You can't spawn on GM island!");
            return true;
        }
    ...
    static bool HandleNpcDeleteCommand(ChatHandler* handler, const char* args)
    {
        ...
        uint32 unitId = unit->GetEntry();

        if (!handler->GetSession()->GetPlayer()->CanDeleteNPC(unitId) && !handler->GetSession()->GetPlayer()->IsAdmin())
        {
            handler->PSendSysMessage("You cannot delete this NPC.");
            return true;
        }
    ...
    */
    ...
    static bool HandleNpcAddTempSpawnCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (!handler->GetSession()->GetPlayer()->IsAdmin())
        {
            ...
            {
                handler->PSendSysMessage("You may only spawn NPCs once every 15 seconds.");
                return true;
            }
            else
            {
