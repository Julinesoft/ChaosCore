...
public:
    ...
    ChatCommand* GetCommands() const
    {
        static ChatCommand modifyCommandTable[] =
        {
            ...
            { "level",          SEC_GAMEMASTER,     false, &HandleModifyLevelCommand,         "", NULL },
    ...
    static bool HandleModifyTalentCommand (ChatHandler* handler, const char* args)
    {
        ...
        {
            if (target->ToPlayer()->NoModify() && !handler->GetSession()->GetPlayer()->IsAdmin() && handler->GetSession()->GetPlayer() != target)
            {
                handler->PSendSysMessage("This player has disabled GM commands being used on them.");
                return true;
            }
    ...
    static void HandleCharacterLevel(Player* player, uint64 player_guid, uint32 oldlevel, uint32 newlevel)
    {
        if (player)
        {
    ...
    static bool HandleModifyLevelCommand(ChatHandler* handler, const char *args)
    {
        ...
        if (target->NoModify() && handler->GetSession()->GetPlayer() != target && !handler->GetSession()->GetPlayer()->IsAdmin())
        {
            handler->PSendSysMessage("This player has disabled gm commands being used on them.");
            return true;
        }
        ...
        if (newlevel > 80)                         // hardcoded maximum level
            newlevel = 80;
