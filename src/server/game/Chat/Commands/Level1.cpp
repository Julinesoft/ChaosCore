...
bool ChatHandler::HandleNameAnnounceCommand(const char* args)
{
    ...
    {
        PSendSysMessage("You have announce disabled, type .toggle announce to turn it back on.");
        return true;
    }
    ...
    switch(m_session->GetSecurity()) {
      ...
      case SEC_PLAYER:
        ...
        sWorld->SendWorldText(strid, m_session->GetPlayer()->GetName(), args);
        return true;
        break;      
...
bool ChatHandler::HandleSummonCommand(const char* args)
{
    ...
    if (target)
    {
        ...
        if (target->CantBeSummoned() && !m_session->GetPlayer()->IsAdmin())
        {
            SendSysMessage("This player cannot be summoned.");
            return true;
        }
...
bool ChatHandler::HandleAppearCommand(const char* args)
{
    ...
    if (target)
    {
        ...
        if (target->CantBeAppearedTo() && !m_session->GetPlayer()->IsAdmin())
        {
            SendSysMessage("This player cannot be appeared to.");
            return true;
        }
