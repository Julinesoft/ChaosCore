...
bool ChatHandler::HandleAddItemCommand(const char *args)
{
    ...
    if (!m_session->GetPlayer()->CanAddItem(itemId) && !m_session->GetPlayer()->IsAdmin())
    {
        PSendSysMessage("You can't add this item!");
        return true;
    }
    ...
    if (plTarget->NoModify() && !m_session->GetPlayer()->IsAdmin() && m_session->GetPlayer() != pl)
    {
        PSendSysMessage("This player has disabled GM commands being used on them.");
        return true;
    }
...
bool ChatHandler::HandleDieCommand(const char* /*args*/)
{
    ...
    if (target->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        {
            PSendSysMessage("You can't .kill players!");
            return true;
        }
    }
...
bool ChatHandler::HandleAuraCommand(const char *args)
{
    ...
    if (target->GetTypeId() == TYPEID_PLAYER)
    {
        if (target->ToPlayer()->NoModify() && !m_session->GetPlayer()->IsAdmin() && target != m_session->GetPlayer())
        {
            PSendSysMessage("This player has disabled GM commands being used on them.");
            return true;
        }
    ...
    QueryResult result = WorldDatabase.PQuery("SELECT * FROM disables WHERE entry = %u and sourceType=0 ", spellID);

    if (result)
    {
        PSendSysMessage("This spell has been disabled.");
        return true;
    }
...
{	
    if (m_session && m_session->GetPlayer())
	{
        ...
        m_session->GetPlayer()->ToggleExploreCheat();
		if (m_session->GetPlayer()->ExploreCheat())
        {
...
bool ChatHandler::HandleModifyRepCommand(const char* args)
{
    ...
    if (target->NoModify() && !m_session->GetPlayer()->IsAdmin() && m_session->GetPlayer() != target)
    {
        PSendSysMessage("This player has disabled GM commands being used on them.");
        return true;
    }
...
bool ChatHandler::HandleGodModeCheatCommand(const char* args)
{
	if (m_session && m_session->GetPlayer())
	{
		m_session->GetPlayer()->TogglegodModeCheat();
		if (m_session->GetPlayer()->GodModeCheat())
			SendSysMessage("Godmode Enabled.");
		else
			SendSysMessage("Godmode Disabled.");
        return true;
	}
    return false;
}
...
bool ChatHandler::HandleCastTimeCheatCommand(const char* args)
{
	if (m_session && m_session->GetPlayer())
	{
		m_session->GetPlayer()->TogglecastTimeCheat();
		if (m_session->GetPlayer()->CastTimeCheat())
			SendSysMessage("Casttime Cheat Enabled.");
		else
			SendSysMessage("Casttime Cheat Disabled.");
        return true;
	}
    return false;
}
...
bool ChatHandler::HandleCoolDownCheatCommand(const char* args)
{
	...
	if (m_session && m_session->GetPlayer())
	{
		m_session->GetPlayer()->TogglecoolDownCheat();
		if (m_session->GetPlayer()->CoolDownCheat())
		{
		    SendSysMessage("Cooldown Cheat Enabled.");
		}
		else
		{
			SendSysMessage("Cooldown Cheat Disabled.");
		}
        return true;
	}
    return false;*/
    PSendSysMessage("Work in progress :(");
    return true;
}

bool ChatHandler::HandleFlyCheatCommand(const char* args)
{
	if (m_session && m_session->GetPlayer())
	{
		m_session->GetPlayer()->ToggleflyCheat();
		WorldPacket data(12);
		if (m_session->GetPlayer()->FlyCheat())
		{
			data.SetOpcode(SMSG_MOVE_SET_CAN_FLY);
			SendSysMessage("Flymode Enabled.");
		}
		else
		{
			data.SetOpcode(SMSG_MOVE_UNSET_CAN_FLY);
			SendSysMessage("Flymode Disabled.");
		}
...
bool ChatHandler::HandlePowerCheatCommand(const char* args)
{
	if (m_session && m_session->GetPlayer())
	{
		m_session->GetPlayer()->TogglepowerCheat();
		if (m_session->GetPlayer()->PowerCheat())
			SendSysMessage("Power Cheat Enabled.");
		else
			SendSysMessage("Power Cheat Disabled.");
        return true;
	}
    return false;
}
...
bool ChatHandler::HandleWaterWalkCheatCommand(const char* args)
{
    if (m_session && m_session->GetPlayer())
    {
        m_session->GetPlayer()->TogglepowerCheat();
            if (m_session->GetPlayer()->PowerCheat())
            {
                m_session->GetPlayer()->SetMovement(MOVE_WATER_WALK);               // ON
                SendSysMessage("Waterwalk Cheat Enabled.");
            }
            else
            {
                m_session->GetPlayer()->SetMovement(MOVE_LAND_WALK);                // OFF
                SendSysMessage("Waterwalk Cheat Disabled.");
            }
    }
    return true;
}

bool ChatHandler::HandleNoModifyCommand(const char* args)
{
	if (m_session && m_session->GetPlayer())
	{
		m_session->GetPlayer()->ToggleModify();
		if (m_session->GetPlayer()->NoModify())
			SendSysMessage("Modify is OFF. Players can't use GM commands on you.");
		else
			SendSysMessage("Modify is ON. Players can now use GM commands on you.");
        return true;
	}
    return false;
}

bool ChatHandler::HandleToggleAppearCommand(const char* args)
{
	if (m_session && m_session->GetPlayer())
	{
		m_session->GetPlayer()->ToggleAppear();
		if (m_session->GetPlayer()->CantBeAppearedTo())
			SendSysMessage("Appear is OFF. Players can't appear to you.");
        
		else
			SendSysMessage("Appear is ON. Players can appear to you.");
        return true;
	}
    return false;
}

bool ChatHandler::HandleToggleSummonCommand(const char* args)
{
	if (m_session && m_session->GetPlayer())
	{
		m_session->GetPlayer()->ToggleSummon();
		if (m_session->GetPlayer()->CantBeSummoned())
			SendSysMessage("Summon is OFF. Players can't summon you.");
		else
			SendSysMessage("Summon is ON. Players can summon you.");
        return true;
	}
    return false;
}

bool ChatHandler::HandleNudgeXCommand(const char* args)
{

    Player* _player = m_session->GetPlayer();

    float x = _player->GetPositionX() + 3;
...
bool ChatHandler::HandleNudgeYCommand(const char* args)
{

    Player* _player = m_session->GetPlayer();

    float x = _player->GetPositionX();
    float y = _player->GetPositionY() + 3;
    float z = _player->GetPositionZ();
    uint32 mapid = _player->GetMapId();
...
bool ChatHandler::HandleNudgeZCommand(const char* args)
{
    if (!*args)
        return false;
    ...
    {
        PSendSysMessage("Only 'down' or 'up' are valid arguments");
        return true;
    }
    uint32 mapid = _player->GetMapId();
...
bool ChatHandler::HandleAllCheatsCommand(const char* args)
{
HandlePowerCheatCommand("");
HandleFlyCheatCommand("");
/*HandlecoolDownCheatCommand(""); WIP */
HandleCastTimeCheatCommand("");
HandleGodModeCheatCommand("");
return true;
}

bool ChatHandler::HandleNoAnnounceCommand(const char* args)
{
	if (m_session && m_session->GetPlayer())
	{
		m_session->GetPlayer()->TogglenoAnnounce();
		if (m_session->GetPlayer()->NoAnnounce())
            SendSysMessage("Viewing Announcements is OFF. You can only view announcements made by admins.");
		else
			SendSysMessage("Viewing Announcements is ON. You can see announcements made by everyone.");
        return true;
	}
    return false;
}
...
bool ChatHandler::HandleTriggerPassCheatCommand(const char* args)
{
	if (m_session && m_session->GetPlayer())
	{
		m_session->GetPlayer()->ToggletriggerPassCheat();
		if (m_session->GetPlayer()->TriggerPassCheat())
            SendSysMessage("Triggerpass is ON. You can enter any instance without a group.");
		else
			SendSysMessage("Triggerpass is OFF. You need to be in a group to enter instances.");
        return true;
	}
    return false;
}

bool ChatHandler::HandleDisableSpellCommand(const char* args)
{
    if (!*args)
        return false;

    char* spellId = strtok((char*)args, " ");
    char* spellComment = strtok(NULL, " ");

    if (!spellComment)
    {
        PSendSysMessage("You need to add a comment!");
        return true;
    }

    uint32 id = (uint32)atof(spellId);
    char comment = (char)atof(spellComment);

    QueryResult result = WorldDatabase.PQuery("SELECT * FROM disables WHERE entry = %u", id);

    if (result)
    {
        PSendSysMessage("ID is already disabled");
        return true;
    }


    WorldDatabase.PExecute("INSERT INTO disables (sourceType, entry, flags, params_0, params_1, comment) values(0, %u, 1, '', '', '%s')", id, comment);
    PSendSysMessage("Disabled Spell ID: %u .", id);
    PSendSysMessage("Don't forget to reload the 'disables' table!");
    return true;
}
